    ...
    if (_roads isEqualTo []) then {
        ...
        for "_i" from 1 to 100 do {
            ...
            if !(surfaceIsWater _pos2) then {
                if (
                    {(_pos2 distance _x) < _minDist} count _posArray isEqualTo 0
                     && !(_pos2 call FUNC(nearBase))
                ) then {
        ...
        for "_i" from 1 to 100 do {
            ...
            if (
                ({(_road distance _x) < _minDist} count _posArray) isEqualTo 0
                 && !((getPos _road) call FUNC(nearBase))
            ) then {
